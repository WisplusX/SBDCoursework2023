  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/transport/${vehicleId}`,
        {
          method: "DELETE",
          headers: {
    ...
    >
      <CardContent>
        <Typography variant="h6">
          <strong>Видалити транспорт</strong>
        </Typography>

        <form onSubmit={handleSubmit}>
          <TextField
            label="ID транспорту для видалення"
            name="vehicleId"
            value={vehicleId}
      ...
      <SuccessNotificationSnackbar
        ...
        message="Транспорт успішно видалено"
        onClose={handleCloseSuccessSnackbar}
      />

      <FailNotificationSnackbar
        open={openFailSnackbar}
        message="Помилка. Не вдалось видалити транспорт"
